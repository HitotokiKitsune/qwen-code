...
describe('useQwenAuth', () => {
    ...
    const createMockSettings = (authType) => ({
        merged: {
    ...
    it('should handle auth progress event - polling', () => {
        ...
        expect(result.current.authStatus).toBe('polling');
        expect(result.current.authMessage).toBe('Waiting for user authorization...');
    });
    it('should handle auth progress event - rate_limit', () => {
        ...
        expect(result.current.authMessage).toBe('Too many requests. The server is rate limiting our requests. Please select a different authentication method or try again later.');
    });
    ...
    it('should clean up event listeners when auth type changes', () => {
        ...
        const { rerender } = renderHook(({ settings, isAuthenticating }) => useQwenAuth(settings, isAuthenticating), { initialProps: { settings: qwenSettings, isAuthenticating: true } });
    ...
    it('should clean up event listeners when authentication stops', () => {
        const settings = createMockSettings(AuthType.QWEN_OAUTH);
        const { rerender } = renderHook(({ isAuthenticating }) => useQwenAuth(settings, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Stop authentication
    ...
    it('should reset state when switching from Qwen auth to another auth type', () => {
        ...
        const { result, rerender } = renderHook(({ settings, isAuthenticating }) => useQwenAuth(settings, isAuthenticating), { initialProps: { settings: qwenSettings, isAuthenticating: true } });
    ...
    it('should reset state when authentication stops', () => {
        ...
        const { result, rerender } = renderHook(({ isAuthenticating }) => useQwenAuth(settings, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Simulate device auth
        act(() => {
            handleDeviceAuth(mockDeviceAuth);
        });
    ...
    it('should maintain isQwenAuth flag correctly', () => {
        ...
        const qwenSettings = createMockSettings(AuthType.QWEN_OAUTH);
        const { result: qwenResult } = renderHook(() => useQwenAuth(qwenSettings, false));
        expect(qwenResult.current.isQwenAuth).toBe(true);
        // Test with other auth types
        const geminiSettings = createMockSettings(AuthType.USE_GEMINI);
        const { result: geminiResult } = renderHook(() => useQwenAuth(geminiSettings, false));
        expect(geminiResult.current.isQwenAuth).toBe(false);
        const oauthSettings = createMockSettings(AuthType.LOGIN_WITH_GOOGLE);
        const { result: oauthResult } = renderHook(() => useQwenAuth(oauthSettings, false));
