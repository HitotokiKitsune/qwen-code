...
import { DEFAULT_GEMINI_FLASH_MODEL, getErrorMessage, GitService, isNodeError, logUserPrompt, MessageSenderType, parseAndFormatApiError, GeminiEventType as ServerGeminiEventType, UnauthorizedError, UserPromptEvent, } from '@qwen-code/qwen-code-core';
...
import { mapToDisplay as mapTrackedToolCallsToDisplay, useReactToolScheduler, } from './useReactToolScheduler.js';
...
export const useGeminiStream = (geminiClient, history, addItem, config, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onEditorClose, onCancelSubmit) => {
            ...
            while (true) {
                try {
                    ...
                    if (error instanceof UnauthorizedError) {
                        onAuthError();
                        break; // Auth error, exit loop.
                    }
                    else if (isNodeError(error) && error.name === 'AbortError') {
                        // User cancelled the request, exit loop.
                        break;
                    }
                    else {
                        // For other errors, display the error and retry after a delay.
                        addItem({
                            type: MessageType.ERROR,
                        ...
                        const updateCountdown = () => {
                            setRetryMessage(`An API error occurred. Retrying in ${countdown} seconds... (Attempt ${newRetryCount})`);
                        };
    ...
    useEffect(() => {
        const saveRestorableToolCalls = async () => {
            ...
            const restorableToolCalls = toolCalls.filter((toolCall) => (toolCall.request.name === 'edit' ||
