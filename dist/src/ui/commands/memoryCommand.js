 ...
 */
import { getErrorMessage, loadServerHierarchicalMemory, QWEN_DIR, } from '@qwen-code/qwen-code-core';
...
export const memoryCommand = {
    ...
    subCommands: [
        ...
        {
            name: 'add',
            description: 'Add content to the memory. Use --global for global memory or --project for project memory.',
            kind: CommandKind.BUILT_IN,
            action: (context, args) => {
                if (!args || args.trim() === '') {
                    return {
                ...
                try {
                    ...
                    if (config) {
                        const { memoryContent, fileCount } = await loadServerHierarchicalMemory(config.getWorkingDir(), config.shouldLoadMemoryFromIncludeDirectories()
                            ...
                            : [], config.getDebugMode(), config.getFileService(), config.getExtensionContextFilePaths(), context.services.settings.merged.memoryImportFormat || 'tree', // Use setting or default to 'tree'
